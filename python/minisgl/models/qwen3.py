...
class Qwen3DecoderLayer(BaseOP):
    """Qwen3 解码器层：LayerNorm -> 自注意力（含 QK Norm） -> LayerNorm -> MLP。"""

    def __init__(self, config: ModelConfig, layer_id: int):
        # Qwen3 注意力层：启用 QK LayerNorm，无注意力偏置
        self.self_attn = Qwen3Attn(config, layer_id, has_qk_norm=True)
        self.mlp = Qwen3MLP(config)  # 门控 MLP 层
...
class Qwen3Model(BaseOP):
    """Qwen3 Transformer 主干网络：词嵌入 -> 多层解码器 -> 最终 RMSNorm。"""
...
class Qwen3ForCausalLM(BaseLLMModel):
    """Qwen3 因果语言模型：主干网络 + LM Head（输出 logits）。"""
