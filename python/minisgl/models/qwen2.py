...
class Qwen2DecoderLayer(BaseOP):
    """Qwen2 解码器层：LayerNorm -> 自注意力（含偏置） -> LayerNorm -> MLP。"""

    def __init__(self, config: ModelConfig, layer_id: int):
        # Qwen2 注意力层：无 QK LayerNorm，但使用注意力偏置（has_attn_bias=True）
        self.self_attn = Qwen2Attn(config, layer_id, has_qk_norm=False, has_attn_bias=True)
        self.mlp = Qwen2MLP(config)  # 门控 MLP 层
...
class Qwen2Model(BaseOP):
    """Qwen2 Transformer 主干网络：词嵌入 -> 多层解码器 -> 最终 RMSNorm。"""
...
class Qwen2ForCausalLM(BaseLLMModel):
    """Qwen2 因果语言模型：主干网络 + LM Head（输出 logits）。"""

    def __init__(self, config: ModelConfig):
        self.model = Qwen2Model(config)  # Transformer 主干网络
